...
class SaleServiceLine(models.Model):
    """ Copy function from sale order line to sale service line """
    ...
    def write(self, values):
        ...
        result = super(SaleServiceLine, self).write(values)
        return result
    ...
    invoice_lines = fields.Many2many('account.move.line', 'sale_service_line_invoice_rel', 'order_line_id', 'invoice_line_id', string='Invoice Lines', copy=False)
    ...
    def _name_search(self, name, args=None, operator='ilike', limit=100, name_get_uid=None):
        ...
        return super(SaleServiceLine, self)._name_search(name, args=args, operator=operator, limit=limit, name_get_uid=name_get_uid)
    ...
    def unlink(self):
        ...
        return super(SaleServiceLine, self).unlink()
...
class SaleOrder(models.Model):
    """ Inherit Sale Order to Override total amount """
    _inherit = "sale.order"
    ...
    service_line = fields.One2many('sale.service.line', 'order_id', string='Service Lines', states={'cancel': [('readonly', True)], 'done': [('readonly', True)]}, copy=True, auto_join=True)

    @api.depends('order_line.price_total', 'service_line.price_total')
    ...
    def _create_invoices_split(self, grouped=False, final=False):
        """
        Copied from _create_invoices split service function
        ...
        for order in self:
            ...
            """ Add service vals """
            service_vals = order._prepare_invoice()
            ...
            for line in order.service_line:
                ...
                if line.qty_to_invoice > 0 or (line.qty_to_invoice < 0 and final):
                    if pending_section:
                        service_vals['invoice_line_ids'].append((0, 0, pending_section._prepare_invoice_line()))
                        pending_section = None
                    service_vals['invoice_line_ids'].append((0, 0, line._prepare_invoice_line()))

            if not invoice_vals['invoice_line_ids'] and not service_vals['invoice_line_ids']:
                raise UserError(_('There is no invoiceable line. If a product has a Delivered quantities invoicing policy, please make sure that a quantity has been delivered.'))

            invoice_vals_list.append(invoice_vals)
            service_vals_list.append(service_vals)
        ...
        if not grouped:
            ...
            """ Service Vals """
            new_service_vals_list = []
            for grouping_keys, invoices in groupby(service_vals_list, key=lambda x: [x.get(grouping_key) for grouping_key in invoice_grouping_keys]):
                origins = set()
                payment_refs = set()
                refs = set()
                ref_invoice_vals = None
                ...
                new_service_vals_list.append(ref_invoice_vals)
            
            invoice_vals_list = new_invoice_vals_list
            """ Service Invoice Vals """
            service_vals_list = new_service_vals_list
        ...
        service_moves = self.env['account.move'].sudo().with_context(default_type='out_invoice').create(service_vals_list)
