...
class MrpCostStructure(models.AbstractModel):
    ...
    def get_lines(self, productions):
        # replace function base odoo to improve query to add averhead duration
        ...
        for product in productions.mapped('product_id'):
            ...
            if Workorders:
                # this code improvemnet
                query_str = """SELECT w.operation_id, op.name, partner.name, sum(t.duration), wc.costs_hour, w.overhead_duration
                                ...
                                GROUP BY w.operation_id, op.name, partner.name, t.user_id, wc.costs_hour, w.overhead_duration
                                ORDER BY op.name, partner.name
                            """
                self.env.cr.execute(query_str, (tuple(Workorders.ids), ))
                for op_id, op_name, user, duration, cost_hour, overhead_duration in self.env.cr.fetchall():
                    operations.append([user, op_id, op_name, duration / 60.0, cost_hour, overhead_duration])
                # --------------------
